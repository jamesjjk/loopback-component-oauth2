...
module.exports = function(app, options) {
          ...
          models.users.findByUsernameOrEmail(subject, function(err, user) {
            ...
            if (!user) {
              return done(new AuthorizationError(
                  'Invalid subjects: ' + subject, 'access_denied'));
            }
  ...
  // Body
  passport.use('loopback-oauth2-client-password', new ClientPasswordStrategy(clientLogin));
  ...
  if (options.providers){
    for (var key in options.providers){
      ...
      if(provider.hasOwnProperty('module')){
          hasProvider = true;
          var providerStratagy = require(provider.module)[(provider.strategy || 'Strategy')];
          passport.use(new providerStratagy({
          ...
          function(req, token, refreshToken, profile, done) {
              ...
              models.userIdentities.login(name, 'oAuth 2.0', profile,
                {
                  token: token,
          ...
          app.get(callBackPath,
                  passport.authenticate(name, {
                          ...
                          session: session,
                          successReturnToOrRedirect : option.loginRedirect || '/', //uses returnTo in session
                          failureRedirect: options.loginPage || '/login'
                  }));
          var authPath = (provider.authPath || '/auth/' + name);
          app.get(authPath,
                  passport.authenticate(name, { scope : (provider.scope || ['email', 'profile']), session: session }));
  ...
  oauth2Paths = oauth2Paths.concat(providerPaths);

  app.middleware('parse', oauth2Paths, bodyParser.urlencoded({extended: false}));
  ...
  if (options.loginPath !== false) {
    ...
    app.post(options.loginPath || '/login',
      ...
      {
        flashFailure: flashFailure,
        successReturnToOrRedirect: option.loginRedirect || '/',
        failureRedirect: options.loginPage || '/login' }));
  }
